...
test-api:
    ...
    let server_pid = (bash -c 'pgrep -f "target/debug/dn42-autopeer" | head -n1' | str trim)
    if ($server_pid == "") {
    ...
    try {
        ...
        psql $env.DATABASE_URL -c "SELECT peer_id, asn, peer_name, endpoint, status FROM peers WHERE asn = 4242421234 AND peer_name = 'fra1';"
        print "2. Background application logs (Observability):"
        tail -n 10 /tmp/autopeer_test.log

        print "========== [5] Delete Peer =========="
        let delete_payload = '{"asn": 4242421234, "peer_name": "fra1", "challenge": { "auth": "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOBz+SBn8O1744J1XQ3OpwIGmXnkWR9u8prAF5GfIL0B", "signature": "-----BEGIN SSH SIGNATURE-----\nU1NIU0lHAAAAAQAAADMAAAALc3NoLWVkMjU1MTkAAAAg4HP5IGfw7XvjgnVdDc6nAgaZee\nRZH27ymsAXkZ8gvQEAAAAEZG40MgAAAAAAAAAGc2hhNTEyAAAAUwAAAAtzc2gtZWQyNTUx\nOQAAAEA30znxGld2LXXnl4sfr/CBaLkWS55AEuIJQy86kN8a0GFf1HVWFDzn2lINgr+Ig6\nxBmJALo8VfppQGMjm5arAK\n-----END SSH SIGNATURE-----", "nonce": "dummy_nonce", "expires_at": 9999999999 }}'
...
test: build-test-image
    ...
    print "Starting local test database container..."
    podman run --replace --rm -d --name dn42-test-db --network dn42-test-net -e POSTGRES_PASSWORD=dummy -e POSTGRES_USER=dummy docker.io/library/postgres:15-alpine
    sleep 3sec
    
    print "Running API tests in isolated Rust container..."
    let exit_code = (try {
        podman run --rm --privileged --network dn42-test-net -v (pwd):/workspace -e DATABASE_URL="postgres://dummy:dummy@dn42-test-db:5432/dummy" dn42-autopeer-test just test-api
        0
    } catch { 1 })
